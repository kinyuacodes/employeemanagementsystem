...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header"><h4 class="text-center" style="padding-top:65px !important;">{{ __('Update state') }}</h4></div>

                <div class="card-body">
                    <form method="POST" action="{{ route('states.update',$state->id) }}" novalidate="">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                               <select name="country_id" id="country_id" class="form-control">
                                     @foreach ($countries as $country )
                                        <option  value="{{$country->id}}"{{$country->id==$state->country_id ?'selected':'not'}}>{{$country->id.'-'.$country->country_name}}</option>
                                   @endforeach
                              </select>
                    ...
                    <div class="form-group row mb-0">
                        <div class="col-md-6 offset-md-4">
                            ...
                            <a href="{{route('states.index')}}" class="btn btn-success" >Back to states</a>
                        </div>
                    </div>
                </div>
            </div>
